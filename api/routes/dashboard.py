...
def dashboard_stats(
    ...
    stats = (
        db.query(
            func.count(
                case(
                    (Document.sender_org_id == org_id, 1)
                )
            ).label("sent"),

            func.count(
                case(
                    (Document.recipient_org_id == org_id, 1)
                )
            ).label("received"),
    ...
    return {
        ...
        "received_change_pct": calculate_change(
            stats.received_current,
            stats.received_previous,
        ),
        "pending_change_pct": calculate_change(
            stats.pending_current,
            stats.pending_previous,
        ),
